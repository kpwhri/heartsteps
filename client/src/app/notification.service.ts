...
export class NotificationService {
    ...
    setup(): Promise<void> {
        this.messageService.opened.subscribe((message: Message) => {
            ...
            this.processOpenedMessage(message).then(() => {
                console.log(
                    "AppNotificationService: Opened message id=" + message.id
                );
                message.toggleOpened();
            });
        });
    ...
    public processOpenedMessage(message: Message): Promise<boolean> {
        switch (message.type) {
            case "weekly-reflection":
                return this.weeklySurveyService
                    .processMessage(message)
                    .then(() => {
                        return this.router.navigate(["weekly-survey"]);
                    });
            case "morning-message":
                return this.morningMessageService
                    .processMessage(message)
                    .then(() => {
            ...
            default:
                console.log(
                    "AppNotificationService: Try to open notification page for: " +
                        message.id
                );
