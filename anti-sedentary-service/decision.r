    ...
    tryCatch(
    {
        ...
        if(return_default) {
           ...
           write(x = temp, file = "./data/errorfile.log", ncolumns = length(temp), append = TRUE)
            ...
            reasons = paste(reasons, 'A and rho not initialized; ', sep = "")
            temp = c(as.vector(unlist(input)), reasons)
            write(x = temp, file = "./data/errorfile.log", ncolumns = length(temp), append = TRUE)
            ...
            if( any(is.element(user.data$time,current.time)) ) {
                ...
                block.steps = unlist(lapply(hour,buckets, FUN = which.block))
                ...
                current.block = which.block(current.hour,buckets)
                ...
                in.block = unlist(lapply(hour(with_tz(current.day.user.data$time, "GMT"))+ (min.hours - beginning.hour),buckets, FUN = which.block)) == current.block
                ...
                if(any(H.t$old.A[H.t$time.diff< 60] == 1) | input$available == 0) {
                    ...
                    rho.t = randomization.probability(N, current.state, remaining.time.in.block, current.run.length, current.hour, H.t, lambda, eta,r_min_x.table,r_minus_x_plus.table,fraction.df)
