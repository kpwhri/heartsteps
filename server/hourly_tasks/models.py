...
class HourlyTask(models.Model):
    ...
    # task = models.OneToOneField(PeriodicTask, blank=True, on_delete=models.CASCADE)
    ...
    def create_hourly_task(user, category, task, name, arguments, minute=0):
        hourly_task = HourlyTask.objects.create(
            user = user,
            category = category
        )
        hourly_task.create_task(
            task = task,
            name = name,
            arguments = arguments
        )
        hourly_task.set_time(
            minute = minute
        )
        return hourly_task
    ...
    def set_time(self, minute):
        ...
        self.task.crontab.minute = minute
