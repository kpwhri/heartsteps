...
class FirstBoutPlanningTime(models.Model):
    ...
    def create(user, time="07:00"):
        ...
        """
        user_obj = FirstBoutPlanningTime.convert_to_user_obj(user)
        assert isinstance(time, str), "time argument should be a string: {}".format(type(time))
        assert FirstBoutPlanningTime.__is_formatted_properly(time), "time should be correctly formated: {}".format(time)
        ...
        return FirstBoutPlanningTime.objects.create(user=user_obj, hour=hour, minute=minute)
    ...
    def update(user:User, time:str):
        ...
        FirstBoutPlanningTime.objects.filter(user=user_obj).update(hour=hour, minute=minute)
        
        return FirstBoutPlanningTime.get(user_obj)
