...
def export_burst_walking_survey(user,directory = None, filename = None, start=None, end=None, from_scratch=True, DEBUG=True):
    ...
    # Get walking suggestion survey objects
    query = WalkingSuggestionSurvey.objects.filter(user_id=uid).order_by('created', 'updated').all()
    df = pd.DataFrame({'Object': [x for x in query]})
